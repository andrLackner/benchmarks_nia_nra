...
; Benchmark: 		cubes_denorm_inv4_len2_1
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 + 3*n^2 - k + 3*n) * (c*k^3*m^2 + c^3*k*m^2)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ 1 (* 3 (+ n 1) (+ n 1))) (+ k m)) (* 3 (+ n 1))) (+ (* (+ c k) (+ k m) (+ k m) (+ k m) (+ m 6) (+ m 6)) (* (+ c k) (+ c k) (+ c k) (+ k m) (+ m 6) (+ m 6)))) 0))
			)
		)
	)
)
