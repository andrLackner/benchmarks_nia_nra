...
; Benchmark: 		cubes_denorm_inv4_len3_5
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 + 3*n^2 - k + 3*n) * (-c*k*m*n - c^3*k + c^3*k^3)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ 1 (* 3 (+ n 1) (+ n 1))) (+ k m)) (* 3 (+ n 1))) (+ (- (* (- 1) (+ c k) (+ k m) (+ m 6) (+ n 1)) (* (+ c k) (+ c k) (+ c k) (+ k m))) (* (+ c k) (+ c k) (+ c k) (+ k m) (+ k m) (+ k m)))) 0))
			)
		)
	)
)
