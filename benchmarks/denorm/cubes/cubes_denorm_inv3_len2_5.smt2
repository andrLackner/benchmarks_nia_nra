...
; Benchmark: 		cubes_denorm_inv3_len2_5
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 - 3*c + k*n - k + 2*n) * (c*k^2*m - c^2*k^3*m^2*n^3)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ (- 1 (* 3 (+ c k))) (* (+ k m) (+ n 1))) (+ k m)) (* 2 (+ n 1))) (- (* (+ c k) (+ k m) (+ k m) (+ m 6)) (* (+ c k) (+ c k) (+ k m) (+ k m) (+ k m) (+ m 6) (+ m 6) (+ n 1) (+ n 1) (+ n 1)))) 0))
			)
		)
	)
)
