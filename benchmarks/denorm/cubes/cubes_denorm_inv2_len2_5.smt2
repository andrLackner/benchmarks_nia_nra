...
; Benchmark: 		cubes_denorm_inv2_len2_5
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (4*1 - 18*c - 9*c*n - 5*k + 9*n + k^2) * (c^3*m^3*n^2 + n)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (* 4 1) (* 18 (+ c k)) (* 9 (+ c k) (+ n 1)) (* 5 (+ k m))) (* 9 (+ n 1)) (* (+ k m) (+ k m))) (+ (* (+ c k) (+ c k) (+ c k) (+ m 6) (+ m 6) (+ m 6) (+ n 1) (+ n 1)) n 1)) 0))
			)
		)
	)
)
