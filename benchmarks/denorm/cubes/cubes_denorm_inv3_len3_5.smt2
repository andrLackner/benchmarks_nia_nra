...
; Benchmark: 		cubes_denorm_inv3_len3_5
...
; 		 1 - 3*c + k*n - k + 2*n
; 		 1 + 3*n^2 - k + 3*n
; Testing invariant: (1 - 3*c + k*n - k + 2*n) * (m^2 - c*m^2*n + m^2*n)
;--------------------------------------------------------------------------
...
(assert 
	(exists
		...
		(and
			...
			(or
				(not (= (* (+ (- (+ (- 1 (* 3 (+ c k))) (* (+ k m) (+ n 1))) (+ k m)) (* 2 (+ n 1))) (+ (- (* (+ m 6) (+ m 6)) (* (+ c k) (+ m 6) (+ m 6) (+ n 1))) (* (+ m 6) (+ m 6) (+ n 1)))) 0))
			)
		)
	)
)
